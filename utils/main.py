...
def train(to_test: list, ct_loader, trainer):
...
def main(to_test: list, device: int = 3, slice: str = None, 
    ...
    dir = set_home(device)
    if from_tensors:
        ct_loader = CTLoaderTensors(data_dir = dir, kfold = True, **kwargs)
    elif slice_range is not None:
        assert slice is not None
        ct_loader = CTLoader2D(slice, kfold = True, slice_range = slice_range, data_dir = dir, 
                    pad = pad, **kwargs)
        for model in to_test:
            model.set_slice_info(slice_range, slice)
    else:
        ct_loader = CTLoader(data_dir = dir, kfold = True, **kwargs)
    trainer = Trainer(batch_size = 32)
    train(to_test, ct_loader, trainer)
