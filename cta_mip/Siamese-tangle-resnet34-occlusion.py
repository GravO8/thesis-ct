...
def out_features(net: str):
    return {"efficientnet_b0":1280, "efficientnet_b1":1280, "efficientnet_b2":1408, "resnet18":512, "resnet34":512, "resnet50":2048}[net]
...
class Siamese(torch.nn.Module):
    def __init__(self, in_channels = 1, transfer: bool = False):
        ...
        chans = out_features(NET)
        self.group_conv = torch.nn.Conv2d(in_channels   = chans*2, 
                                           out_channels = chans, 
                                           groups       = chans,
                                           kernel_size  = 3, 
                                           padding      = 1)
...
class LitModel(pl.LightningModule):
    ...
    def __init__(self, input_shape, num_classes, learning_rate = 2e-4, weight_decay = 0, transfer = False):
        ...
        # self.feature_extractor = timm.create_model(NET, pretrained = True)
        # if "resnet" in NET:
        ...
        self.classifier = torch.nn.Linear(out_features(NET), self.num_classes)
...
if __name__ == "__main__":
    ...
    for NET in ("efficientnet_b0", "efficientnet_b1", "efficientnet_b2", "resnet18", "resnet34", "resnet50"):
        name    = f"Siamese-tangle-{NET}-{target}"
