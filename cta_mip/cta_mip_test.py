...
class MIPTest(torch.utils.data.Dataset):
    def __init__(self, set: str, dir: str = "", csv_file: str = "dataset.csv",
    target: str = "binary_rankin", augmentations: list = [], in_channels: int = 1):
        assert set in ("train", "all", "test")
...
class MIPTestDataModule(pl.LightningDataModule):
    def __init__(self, batch_size, csv_file: str, dir: str = "", 
    ...
    def setup(self, stage = None):
        self.train = MIPTest("train", **self.kwargs)
        self.test  = MIPTest("test",  **self.kwargs)
        self.all   = MIPTest("all",   **self.kwargs)
        # del self.kwargs["augmentations"]
        self.train.dataset.transform = None
        self.test.dataset.transform  = None
        self.all.dataset.transform   = None
    ...
    def val_dataloader(self):
        return DataLoader(self.test, batch_size = self.batch_size, num_workers = 8 if self.cuda else 1, pin_memory = self.cuda)
    def test_dataloader(self):
        return DataLoader(self.test, batch_size = self.batch_size, num_workers = 8 if self.cuda else 1, pin_memory = self.cuda)
    def predict_dataloader(self):
        return DataLoader(self.all,  batch_size = self.batch_size, shuffle = False, num_workers = 8 if self.cuda else 1, pin_memory = self.cuda)
...
class LitModel(pl.LightningModule):
    ...
    def __init__(self, input_shape, num_classes, learning_rate = 2e-4, weight_decay = 0, transfer = False):
        ...
        self.classifier = torch.nn.Linear({ "efficientnet_b0":1280,
                                            "efficientnet_b1":1280, 
                                            "efficientnet_b2":1408,
                                            "resnet18": 512,
                                            "resnet34": 512,
                                            "resnet50": 2048}[NET],self.num_classes)
        # self.criterion  = BinaryFocalLoss(alpha = .75, gamma = 2, reduction = "mean")
...
if __name__ == "__main__":
    ...
    DATASET = f"dataset-{target}.csv"
    for NET in ("efficientnet_b0", "efficientnet_b1", "efficientnet_b2", "resnet18", "resnet34", "resnet50"):
        name    = f"Siamese-cut-{NET}-{target}"
        with open(f"{name}-test.csv", "w+") as f:
            f.write("fold,auc,f1,accuracy\n")
            checkpoint_callback = pl.callbacks.ModelCheckpoint(monitor = "val/loss", mode = "min")
            trainer = pl.Trainer(logger = WandbLogger(project = "mip4", name = f"{name}-test"), 
                                max_epochs = 150, 
                                accelerator = "gpu" if INESC else "cpu",
                                log_every_n_steps = 20,
                                callbacks = [checkpoint_callback])
            data = MIPTestDataModule(32, DATASET, DIR, augmentations = [], target = target)
            ...
            performance = trainer.test(model, datamodule = data, ckpt_path = "best")[0]
            f.write(f"test,{performance['test/auc']},{performance['test/f1']},{performance['test/acc']}\n")
            
            batched_predictions = trainer.predict(model, datamodule = data, ckpt_path = "best")
            predictions = []
            for batch in batched_predictions:
                predictions.extend(batch.numpy())
            name = os.path.join(DIR,DATASET)
