...
class Siamese(torch.nn.Module):
    ...
    def forward(self, x):
        x1 = self.encoder(x)
        x1 = self.pooling(x1).squeeze()
        x2 = self.encoder(x.flip(3))
        x2 = self.pooling(x2).squeeze()
        x  = torch.abs(x1 - x2)
        return x
...
class LitModel(pl.LightningModule):
    ...
    def __init__(self, input_shape, num_classes, learning_rate = 2e-4, weight_decay = 0, transfer = False):
        ...
        self.classifier = torch.nn.Linear({"efficientnet_b0":1280, "efficientnet_b1":1280, "efficientnet_b2":1408}[NET], self.num_classes)
...
if __name__ == "__main__":
    ...
    for NET in ("efficientnet_b0", "efficientnet_b1", "efficientnet_b2"):
        name    = f"Siamese-after-{NET}-{target}"
